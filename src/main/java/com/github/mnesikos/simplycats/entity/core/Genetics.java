...
public class Genetics {
    ...
    public static String getPhenotypeDescription(CompoundTag nbt, boolean includeSex) {
        ...
        TranslatableText base = new TranslatableText("cat.base." + eumelanin + (phaeomelanin.equals(Phaeomelanin.NOT_RED.toString().toLowerCase()) ? "" : "_" + phaeomelanin) + ".name");
        ...
        if (dilute) {
            base = new TranslatableText("cat.base." + eumelanin + "_" + phaeomelanin + "_" + dilution + ".name");
            if (caramelized)
                base = new TranslatableText("cat.base." + eumelanin + "_" + phaeomelanin + "_" + diluteMod + ".name");
        }
        boolean red = phaeomelanin.equals(Phaeomelanin.RED.toString().toLowerCase());
        if (red) {
            base = new TranslatableText("cat.base." + phaeomelanin + ".name");
            if (dilute) {
                base = new TranslatableText("cat.base." + phaeomelanin + "_" + dilution + ".name");
                if (caramelized)
                    base = new TranslatableText("cat.base." + phaeomelanin + "_" + diluteMod + ".name");
            }
        }
        ...
        if (!white.equals(White.NONE.toString().toLowerCase())) {
            if (white.equals(White.DOMINANT.toString().toLowerCase()) || nbt.getString("White_0").contains("6")) {
                whiteText = new TranslatableText("cat.white.solid_white.name");
                return whiteText.formatted() + (includeSex ? (" " + sex) : "");
            }
            if (nbt.getString("White_0").contains("5")) {
                whiteText = new TranslatableText("cat.white.mostly_white.name");
                return whiteText.formatted() + " " + base.formatted() +
                        (tabby.formatted().equals("") ? "" : " " + tabby.formatted()) +
                        (point.formatted().equals("") ? "" : " " + point.formatted()) +
                        (includeSex ? (" " + sex) : "");
            } else
                whiteText = new TranslatableText("cat.white.some_white.name");
        }

        return base.formatted() +
                (tabby.formatted().equals("") ? "" : " " + tabby.formatted()) +
                (point.formatted().equals("") ? "" : " " + point.formatted()) +
                " " + whiteText.formatted() + (includeSex ? (" " + sex) : "");
    }
    ...
    public enum EyeColor {
        ...
        public static EyeColor getByString(String string) {
            switch (string) {
                case "copper":
                    return COPPER;
                case "gold":
                ...
                case "blue":
                    return BLUE;
                default:
                    throw new IllegalArgumentException("Invalid eye color value: " + string);
            }
        }
    }
